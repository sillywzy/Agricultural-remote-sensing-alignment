"""Train ParcelAlign v2.1: farmland-boosted sampling + amplified gate scale; warm start from v2.
...
INIT_FROM = os.path.join(ROOT, "outputs", "parcel-align", "run_v2_20260920_211323", "best_trainable.pt")
FARMLAND_BOOST = 4
...
OUT = os.path.join(ROOT, "outputs", "parcel-align", "run_v21_" + time.strftime("%Y%m%d_%H%M%S"))
...
class RSICDPairs(Dataset):
    def __init__(self, imgdir, pre, farmland_boost=0):
        data = json.load(open(TRAIN_JSON, encoding="utf-8"))["images"]
        items = [(e["filename"], [s["raw"] for s in e["sentences"]])
                 for e in data if e["split"] == "train"]
        if farmland_boost > 0:
            extra = [it for it in items if it[0].startswith("farmland_")] * farmland_boost
            items = items + extra
        self.items = items
...
if __name__ == "__main__":
    set_seed(SEED)
    log(f"output: {OUT}\ndevice: {DEVICE}")
    json.dump({"seed":SEED,"epochs":EPOCHS,"bs":BS,"lr":LR,"wd":WD,"k":K,"farmland_boost":FARMLAND_BOOST,"gate_scale_init":2.0,"init_from":INIT_FROM},
    ...
    pa = ParcelAlignV2(model, k=K, scale_init=2.0).to(DEVICE)
    if os.path.exists(INIT_FROM):
        ck = torch.load(INIT_FROM, map_location="cpu")
        pa.tokenizer.load_state_dict(ck["tokenizer"])
        pa.gate.load_state_dict(ck["gate"], strict=False)
        pa.logit_g.data = ck["logit_g"].to(DEVICE)
        pa.logit_p.data = ck["logit_p"].to(DEVICE)
        log(f"warm start from v2: {INIT_FROM} | gate scale init {pa.gate.scale.item():.2f}")
    ...
    ds = RSICDPairs(TRAIN_IMGDIR, pre, FARMLAND_BOOST)
    dl = DataLoader(ds, batch_size=BS, shuffle=True, num_workers=0, collate_fn=collate)
    n_farm = sum(1 for it in ds.items if it[0].startswith("farmland_"))
    log(f"train: {len(ds)} pairs ({n_farm} farmland = {n_farm/len(ds)*100:.1f}%), {len(dl)} steps/epoch")
